  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (interaction.commandName === 'stats') {
      ...
      if (!(await hasRole(guild, interaction.user.id))) {
        return interaction.reply({ content: '❌ **Premium+** members only.', flags: MessageFlags.Ephemeral });
      }
    ...
    if (interaction.commandName === 'admin') {
      if (interaction.user.id !== interaction.guild.ownerId) {
        return interaction.reply({ content: '❌ Server owner only.', flags: MessageFlags.Ephemeral });
      }
    ...
    if (interaction.commandName === 'block') {
      if (!(await hasRole(interaction.guild, interaction.user.id))) return interaction.reply({ content: '❌ **Premium+** only.', flags: MessageFlags.Ephemeral });
      await db.upsertUser(interaction.user.id, interaction.user.username);
      const type = interaction.options.getString('type');
      const value = interaction.options.getString('value');
      await db.addToBlocklist(interaction.user.id, type, value);
      return interaction.reply({ content: `🚫 Blocked **${type}**: ${value}`, flags: MessageFlags.Ephemeral });
    }

    if (interaction.commandName === 'unblock') {
      const id = interaction.options.getInteger('id');
      await db.removeFromBlocklist(interaction.user.id, id);
      return interaction.reply({ content: `✅ Removed from blocklist.`, flags: MessageFlags.Ephemeral });
    }
    ...
    if (interaction.commandName === 'setteams') {
      ...
      return interaction.reply({ content: `✅ Custom team IDs saved!`, flags: MessageFlags.Ephemeral });
    }
  }
